...
//   • API_BASE → dark-horse-v23.onrender.com
//   • Endpointها → /api/v2/darkhorse/...
//   • فایل سوالات → questions_v2.json
//   • اضافه شدن صفحه انتخاب بین هدایت تحصیلی و انتخاب رشته دانشگاهی

// ==================== تنظیمات پایه ====================
const DATA_BASE = './data/';                                    // پوشه داده‌های فرانت‌اند
const API_BASE = 'https://dark-horse-v23.onrender.com';         // ✅ آدرس API جدید (v23)
...
async function loadMicroMotivesMap() {
  try {
    ...
    // ساختار فایل: لیستی از { code, description_fa }
...
async function loadQuestions() {
  try {
    ...
    // ساختار فایل questions_v2.json: { layer_2_strategies, layer_3_values }
    state.strategyQuestions = data.layer_2_strategies.questions;   // ۲۵ سوال راهبردی
    state.valueQuestions = data.layer_3_values.questions;          // ۱۵ سوال ارزشی
...
function displayResults(data, type) {
  ...
  let html = `
    ...
    <p>بر اساس <strong>${state.likedCodes.length}</strong> خرده‌انگیزه،

  // نمایش شاخص اطمینان سراسری
  const confidence = data?.discovery_result?.confidence || data?.branch_discovery_result?.confidence;
  if (confidence && confidence.level) {
    html += `
      <div style="background:#1a1a2e;border:1px solid #f0c040;border-radius:12px;padding:15px;margin:15px 0;text-align:right;">
        <p style="color:#f0c040;font-weight:bold;">📊 شاخص اطمینان: ${confidence.level}</p>
        <p style="color:#b0a080;font-size:0.9rem;">${confidence.interpretation || ''}</p>
        ${confidence.rank1_name ? `<p style="color:#b0a080;font-size:0.85rem;">🥇 رتبه ۱: ${escapeHtml(confidence.rank1_name)} (${confidence.rank1_score}%)</p>` : ''}
        ${confidence.rank2_name ? `<p style="color:#b0a080;font-size:0.85rem;">🥈 رتبه ۲: ${escapeHtml(confidence.rank2_name)} (${confidence.rank2_score}%)</p>` : ''}
      </div>
    `;
  }
    ...
    matched.forEach(r => {
      ...
      html += `
        <div class="card" style="text-align:right;">
          ...
          ${alignmentBadge}
      ${(() => {
        const conflict = r.internal_conflict || {};
        const gradient = r.gradient_alignment || {};
        const insight = r.individuality_insight || '';
        let conflictHtml = '';
        if (conflict.level || conflict.type_label) {
          const severityColor = {
            'very_low': '#4caf50',
            'low': '#8bc34a',
            'moderate': '#ffeb3b',
            'high': '#ff9800',
            'severe': '#f44336'
          }[conflict.severity] || '#f0c040';
          conflictHtml = `
            <div style="margin-top:10px;padding:10px;background:#0a0a0f;border-radius:8px;border-right:3px solid ${severityColor};">
              <p style="margin:0;font-size:0.85rem;color:#b0a080;">
                ⚖️ <strong>تعارض درونی:</strong> 
                <span style="color:${severityColor};">${conflict.severity_label || conflict.level || 'نامشخص'}</span>
                ${conflict.type_label ? ` (${conflict.type_label})` : ''}
                ${conflict.index !== undefined ? ` — شاخص: ${conflict.index.toFixed(2)}` : ''}
              </p>
              ${gradient.index !== undefined ? `
                <p style="margin:0;font-size:0.85rem;color:#b0a080;">
                  📈 <strong>گرادیان صعود:</strong> ${gradient.level || ''} (${(gradient.index * 100).toFixed(0)}%)
                </p>
              ` : ''}
            </div>
          `;
        ...
        if (insight) {
          insightHtml = `
            <div style="margin-top:8px;padding:10px;background:#1a1a2e;border-radius:8px;border:1px solid #d4af37;text-align:right;font-size:0.85rem;line-height:1.9;">
              <p style="margin:0;color:#d4af37;">💡 ${escapeHtml(insight)}</p>
            </div>
          `;
        }
        return conflictHtml + insightHtml;
      })()}
...
// ==================== راه‌اندازی اولیه ====================
async function init() {
  // بارگذاری موازی سوالات و خرده‌انگیزه‌ها
  await Promise.all([loadQuestions(), loadMicroMotivesMap()]);

  // ✅ محافظ صفحه سیاه: اگر داده‌ها بارگذاری نشدند، پیام واضح نمایش بده
  if (!state.questionsReady || !state.motivesReady) {
    console.error('بارگذاری داده ناموفق بود:', {
      questions: state.questionsReady,
      motives: state.motivesReady
    });
    const container = document.getElementById('app') || document.body;
    container.innerHTML = `
      <div style="text-align:center; padding:60px 24px; color:#fff; font-family:inherit;">
        <div style="font-size:48px; margin-bottom:16px;">⚠️</div>
        <h2 style="margin-bottom:12px;">خطا در بارگذاری داده‌ها</h2>
        <p style="opacity:0.8; margin-bottom:24px;">
          لطفاً اتصال اینترنت خود را بررسی کرده و صفحه را دوباره بارگذاری کنید.
        </p>
        <button onclick="location.reload()"
          style="padding:12px 32px; border:none; border-radius:8px; background:#e94560;
                 color:#fff; font-size:16px; cursor:pointer; font-family:inherit;">
          🔄 بارگذاری مجدد
        </button>
      </div>`;
    return;   // ادامه نده — منتظر بمان کاربر صفحه را رفرش کند
  }

  // اگر نشست قبلی ذخیره شده، بازیابی کن
